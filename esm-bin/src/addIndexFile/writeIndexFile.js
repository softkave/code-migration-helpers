...
import { kIndexFilename, kNamesAndKeywords, kNewline, kTSExtension, } from '../utils/constants.js';
...
export async function writeIndexFile(entries, opts, propagateToParent) {
    ...
    const rEntries = {
        ...entries,
        entries: entries.entries.map(entry => entry.importSrc
            ? { ...entry, importSrc: entry.importSrc }
            : {
                ...entry,
                importSrc: kNamesAndKeywords.dot +
                    path.sep +
                    path.basename(entry.filepath, path.extname(entry.filepath)),
            }),
    };
    const { newExportEntries, exportNodes } = await filterExistingExportSources(sourceFile, rEntries);
    if (exportNodes.length === 0 && !opts.includeEmptyFiles) {
        return;
    }
    const strOrUndefinedEntries = await Promise.all(newExportEntries
        .filter(entry => entry.filepath !== indexFilepath)
        .map(async (entry) => {
        return stringifyExportIdentifiers((await getExportIdentifiers(entry.filepath, opts)) || [], entry.importSrc);
    }));
